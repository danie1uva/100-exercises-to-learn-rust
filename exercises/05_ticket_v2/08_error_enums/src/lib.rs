...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description, status.clone()) {
        ...
        Err(TicketNewError::DescError { error }) => Ticket { title: title, description: "Description not provided".to_string(), status: status } 
    }
}
...
impl Ticket {
    pub fn new(
        ...
        if description.is_empty() {
            return Err(TicketNewError::DescError { error: "Description cannot be empty".to_string() } );
        }
        if description.len() > 500 {
            return Err(TicketNewError::DescError { error: "Description cannot be longer than 500 bytes".to_string() } );
        }
